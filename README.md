...
- Prevent multiple clients from selecting the same console (Artemis 2 parity) (vanilla feature request [#35](https://github.com/artemis-sbs/LegendaryMissions/issues/35))
  - By default, the exclusive consoles are helm, weapons, engineering, science, and comms
  - Which consoles do/don't allow multiple clients is configurable in settings.yaml (which is located each mission's folder, e.g. data/missions/legendarymissions/settings.yaml)
...
Game results screen:
- Display more detailed statistics about the overall game. All displayed game overview stats are:
  - Reason why game ended
  - Difficulty level
  - Map (siege, single front, border war, etc)
  - Offense:
    - Number of vessels destroyed, total and broken down by race
    - Number of vessels surrendered, total and broken down by race
    - Number of surrendered ships killed (Artemis 2 parity)
    - Total number of enemies remaining
  - Losses:
    - Total number of player/friendly vessels lost
	- Number of player ships lost
	- Number of player single-seat craft lost
	- Number of friendly stations lost (Artemis 2 parity)
	- Number of friendly NPC ships lost (Artemis 2 parity)
  - Misc:
    - Number of player beams fired (Artemis 2 parity)
	- Number of successful dock attempts (Artemis 2 parity)
	- Number of anomalies collected (Artemis 2 parity)
	- Mission duration
- Display statistics about each player ship
  - Ship name and type
  - Whether the ship survived or was lost, or number of times the ship was destroyed if player ships respawning is enabled
  - Offense:
    - Number of vessels killed, total and broken down by race
    - Number of vessels surrendered, total and broken down by race
    - Number of surrendered ships killed
  - Friendly Fire:
    - Total number of friendly vessels killed
	- Number of friendly stations killed
	- Number of friendly NPC ships killed
  - Misc:
    - Number of player beams fired
	- Number of successful dock attempts
	- Number of anomalies collected
    - Remaining supplies (energy and ordinance)
    - Looted supplies (energy and ordinance), if the ship could loot
...
Comms:
- Added a hint pointing players to the right-click/long-tap menus
...
- Add zoom and color mode controls to hangar map (vanilla feature request [#603](https://github.com/artemis-sbs/LegendaryMissions/issues/603))
- Do not broadcast messages in text waterfall when a player single-seat craft docks

Mainscreen:
- Renamed to "Mainscreen View"
...
- Log more details when opening the mainscreen console, in hopes it helps diagnose mainscreen client crashes

Other Consoles:
- Added 3d View console
  - Shows a 3d view of the player ship, just like the mainscreen console does while it's in 3d mode
  - Facing and inside/chase/tracking can be changed independently of the ship's mainscreen
- Added Map View console
  - Shows a 2d map view centered on the player ship, just like the mainscreen console does while it's in tactical or LRS mode
- Added Internal View console
  - Shows a view of the internal ship grid, just like the mainscreen console does while it's in data mode
  - Also shows the repair level percentages of each ship system
- Renamed "Cinematic" console to "Cinematic View", to make it clearer to readers that it is view-only
- Renamed "Display Panels" console to "Display Panels View", to make it clearer to readers that it is view-only

Operator mode:
- Allow configuring server to run in quasi-headless mode, in which it will render (almost) nothing on its screen
- Allow configuring which console will show on the operator client after the game starts
...
- Do not end the game when all friendly stations have been destroyed if at least one player ship can loot surrendered ships (i.e. is a pirate)
...
Secret Meeting:
- Use a separate meeting length setting instead of improperly interpreting the game time limit setting
- Enable "Upgrades from wrecks" setting. (Normally, the Upgrades setting influences both upgrades spawned at the start of the game and upgrades spawned from destroying wrecks. In Secret Meeting, this setting only influences upgrades spawned from destroying wrecks.)

These independent mods are included:
- [Unofficial Patch](https://github.com/quaris628/ArtemisCosmosUnofficialPatch) v1.14
...
Mission | Is it ok to run this mission with the Comprehensive Enhancements Mod installed? | Does the Comprehensive Enhancements Mod work? | Comments
--- | --- | --- | ---
...
Mod (& version) | Is it ok to install both mods? | In what order should they be installed? | Would the Comprehensive Enhancements Mod work? | Comments
--- | --- | --- | --- | ---
[Unofficial Patch](https://github.com/quaris628/ArtemisCosmosUnofficialPatch) v1.14 | Yes, but there's no reason to | Unofficial Patch first, Comprehensive Enhancements Mod second | Yes | The Comprehensive Enhancements Mod already includes the Unofficial Patch
...
[Eastern Front Mod](https://github.com/quaris628/ArtemisCosmosEasternFrontMod) v4.2.3 | Yes, but there's no reason to | Comprehensive Enhancements Mod first, Eastern Front Mod second | Yes | The Eastern Front Mod already includes the Comprehensive Enhancements Mod
...
- PirateLord - Pirate ship design and balance

...
from sbs_utils.procedural.gui import gui_button, gui_hide, gui_message, gui_represent, gui_row, gui_section, gui_show, gui_text
...
from game_state import end_game, get_game_state, game_state_paused, resume_game
...
def create_pause_menu(client_id):
    ...
    if current_mission_name is not None and len(current_mission_name) > 0:
        gui_row(style="row-height:64px;")
        restart_current_mission_button = gui_button(f"Reboot {current_mission_name}", style=f"font:gui-3;padding:10px,5px,10px,5px;color:{color_text()};")
    else:
...
def _pause_menu_sync_hide_or_show():
    main_section, title_text, resume_button, end_button, restart_current_mission_button, back_to_startup_mission_button, confirm_section, confirm_message_text, cancel_button, confirm_button, background_image = _get_pause_menu_gui_elements() # pylint: disable=unused-variable
    ...
    gui_represent(background_image)
    gui_represent(main_section)
    gui_represent(title_text)
    gui_represent(resume_button)
    gui_represent(end_button)
    if restart_current_mission_button is not None:
        gui_represent(restart_current_mission_button)
    if back_to_startup_mission_button is not None:
        gui_represent(back_to_startup_mission_button)
    
    confirm_section.background_color = "#00000000"
    gui_hide(confirm_message_text)
    gui_hide(cancel_button)
    gui_hide(confirm_button)
    gui_represent(confirm_section)
    gui_represent(confirm_message_text)
    gui_represent(cancel_button)
    gui_represent(confirm_button)
    _set_confirm_label(None)
...
def _pause_menu_on_confirm_end_game():
    ...
    GAME_STATISTICS.record_game_end(reason="Game was ended manually.", is_success=True)
    end_game()
...
def _pause_menu_on_restart_current_mission_button_clicked():
    ...
    _pause_menu_show_confirm_section(f"Are you sure you want to reboot the {current_mission_name} mission?\nDoing so will unceremoniously end the current game, and reset the simulation setup settings to their defaults.", f"Confirm Reboot {current_mission_name}", _pause_menu_on_confirm_restart_current_mission)
...
def _pause_menu_on_back_to_startup_mission_button_clicked():
    ...
    _pause_menu_show_confirm_section(f"Are you sure you want to go back to {startup_mission_name}?\nDoing so will unceremoniously end the current game, and reset the simulation setup settings to their defaults.", f"Confirm Go Back to {startup_mission_name}", _pause_menu_on_confirm_back_to_startup_mission)

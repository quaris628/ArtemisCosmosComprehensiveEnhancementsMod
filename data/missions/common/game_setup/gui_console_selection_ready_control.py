...
from sbs_utils.procedural.gui import gui_checkbox, gui_hide, gui_message, gui_represent, gui_section, gui_show, gui_text
...
def _on_ready_checkbox_clicked():
    client_id = get_variable("client_id")
    GAME_SETUP_DATA = get_game_setup_data()
    checkbox, checkbox_label, cannot_enter_game_text = _get_ready_control_gui_elements() # pylint: disable=unused-variable
...
def _sync_showing_or_hiding_ready_control(client_id, checkbox, checkbox_label, cannot_enter_game_text):
    ...
    if show and checkbox.is_hidden:
        ...
        gui_hide(cannot_enter_game_text)
    gui_represent(checkbox)
    gui_represent(checkbox_label)
    gui_represent(cannot_enter_game_text)

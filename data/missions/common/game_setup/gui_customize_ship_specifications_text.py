...
from sbs_utils.procedural.gui import gui_blank, gui_hide, gui_represent, gui_row, gui_section, gui_show, gui_sub_section, gui_text
...
from data.missions.common.gui_color_scheme import color_text, color_text_secondary, color_background, color_divider_secondary
...
def create_ship_type_specifications_text(ship, x_left, y_top, x_right, y_bottom):
    ...
    # Table of values
    beam_texts_list = []
    for _ in range(_MAXIMUM_DISPLAYABLE_BEAM_DETAILS_COUNT):
        # The "with" syntax is necessary
        # https://github.com/orgs/artemis-sbs/discussions/586#discussioncomment-16688356
        with gui_sub_section(style="col-width:48px;"):
            beam_texts = []
            for _ in range(len(beam_details_label_strings)):
                gui_row(style="row-height:24px;")
    ...
    gui_row(style="row-height:2px;")
    gui_blank()
    
    shields_row1_value_text, shields_row1_label_text = _create_label_value_pair("")
    shields_row2_value_text, shields_row2_label_text = _create_label_value_pair("")
...
def _sync_ship_type_specifications_on_ship_type_changed():
    ...
    beam_count_text, all_beam_details_label_texts, beam_texts_list, tubes_count_text, all_magazine_label_texts, ordinance_type_max_texts, drive_text, drive_energy_cost_text, speed_text, turn_rate_text, shuttles_text, fighters_text, bombers_text, sensor_strength_text, ship_energy_cost_text, shields_row1_label_text, shields_row1_value_text, shields_row2_label_text, shields_row2_value_text = _get_ship_type_specifications_gui_elements()
    
    # Beams
    
    if len(ship_type.beams) == 0:
        beam_count_text.value = "No Beams"
        gui_represent(beam_count_text)
    ...
    if ship_type.tube_count == 0:
        ...
        gui_represent(tubes_count_text)
        ...
        for magazine_label_text in all_magazine_label_texts:
            ...
            gui_represent(magazine_label_text)
        ordinance_type_label_strings = _get_ordinance_type_label_strings()
        for i in range(4):
            ...
            gui_represent(ordinance_type_max_texts[i])
    ...
    gui_represent(drive_text)
    gui_represent(drive_energy_cost_text)
    
    speed_text.value = f"{_get_speed_description(ship_type.speed_coeff)} ({ship_type.speed_coeff:.0%})"
    gui_represent(speed_text)
    
    turn_rate_text.value = f"{_get_turn_rate_description(ship_type.turn_rate)} ({ship_type.turn_rate:.0%})"
    gui_represent(turn_rate_text)
    
    # Misc
    
    match len(ship_type.shields):
        case 0:
            shields_row1_label_text.value = "No Shields"
            shields_row1_value_text.value = ""
            gui_hide(shields_row2_label_text)
            gui_hide(shields_row2_value_text)
        case 1:
            shields_row1_label_text.value = "One Shield"
            shields_row1_value_text.value = str(ship_type.shields[0])
            gui_hide(shields_row2_label_text)
            gui_hide(shields_row2_value_text)
        case 2:
            shields_row1_label_text.value = "Front Shield"
            shields_row1_value_text.value = str(ship_type.shields[0])
            gui_show(shields_row2_label_text)
            gui_show(shields_row2_value_text)
            shields_row2_label_text.value = "Rear Shield"
            shields_row2_value_text.value = str(ship_type.shields[1])
        case _:
            shields_row1_label_text.value = f"{len(ship_type.shields)} Shields"
            shields_row1_value_text.value = ", ".join(ship_type.shields)
            gui_hide(shields_row2_label_text)
            gui_hide(shields_row2_value_text)
    gui_represent(shields_row1_label_text)
    gui_represent(shields_row1_value_text)
    gui_represent(shields_row2_label_text)
    gui_represent(shields_row2_value_text)
    
    ship_energy_cost_text.value = f"{_get_efficiency_description(ship_type.ship_energy_cost)} ({ship_type.ship_energy_cost:.0%})"
    gui_represent(ship_energy_cost_text)
    
    sensor_strength_text.value = f"{_get_scan_strength_description(ship_type.scan_strength_coeff)} ({ship_type.scan_strength_coeff:.0%})"
    gui_represent(sensor_strength_text)
...
def _get_speed_description(speed):
    if speed < 0.5:
        return "Extraordinarily slow"
    elif speed < 0.618:
        return "Very slow"
    elif speed < 0.8:
        return "Slow"
        ...
        return "Extraordinarily fast"

def _get_turn_rate_description(turn_rate):
    # Right now, speed and turn rate descriptions are idential.
    # This might change in the future though.
    return _get_speed_description(turn_rate)

def _get_scan_strength_description(scan_strength_coeff):
    if scan_strength_coeff < 0.5:
        return "Extraordinarily short"
    elif scan_strength_coeff < 0.618:
        return "Very short"
    elif scan_strength_coeff < 0.8:
        return "Short"
    elif scan_strength_coeff < 1:
        return "Somewhat short"
    elif scan_strength_coeff == 1:
        return "Standard"
    elif scan_strength_coeff <= 1.25:
        return "Somewhat far"
    elif scan_strength_coeff <= 1.618:
        return "Far"
    elif scan_strength_coeff <= 2:
        return "Very far"
    else:
        return "Extraordinarily far"
...
def _set_ship_type_specifications_gui_elements(beam_count_text, all_beam_details_label_texts, beam_texts_list, tubes_count_text, all_magazine_label_texts, ordinance_type_max_texts, drive_text, drive_energy_cost_text, speed_text, turn_rate_text, shuttles_text, fighters_text, bombers_text, sensor_strength_text, ship_energy_cost_text, shields_row1_label_text, shields_row1_value_text, shields_row2_label_text, shields_row2_value_text):
    elements = (beam_count_text, all_beam_details_label_texts, beam_texts_list, tubes_count_text, all_magazine_label_texts, ordinance_type_max_texts, drive_text, drive_energy_cost_text, speed_text, turn_rate_text, shuttles_text, fighters_text, bombers_text, sensor_strength_text, ship_energy_cost_text, shields_row1_label_text, shields_row1_value_text, shields_row2_label_text, shields_row2_value_text)

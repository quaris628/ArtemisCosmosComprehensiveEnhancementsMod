...
from sbs_utils.procedural.gui import gui_blank, gui_button, gui_dropdown_patched, gui_message, gui_hide, gui_represent, gui_section, gui_show, gui_text
...
def _player_ship_type_controls_update_after_delay():
    ...
    gui_represent(ship_origin_dropdown)
    gui_represent(ship_type_name_dropdown)
    gui_represent(next_button)
    gui_represent(previous_button)
...
def _player_ship_type_controls_on_origin_dropdown_changed():
    ...
    ship = get_variable("SHIP")
    ship_type = VESSEL_TYPES_DATA.get_ship_type_from_key(ship.ship_type_key)
    ship_origin_dropdown, ship_type_name_dropdown, next_button, previous_button = _get_ship_type_dropdowns(ship.number) # pylint: disable=unused-variable

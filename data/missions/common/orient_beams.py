...
def has_orientable_beams(ship_id):
    return to_space_object(ship_id).art_id in { ship_type_pirate_strongbow_ef(), ship_type_pirate_brigantine_ef() }

def orient_beams_forward(ship_id):
    ship_type = to_space_object(ship_id).art_id
...
def is_beam_orientation_forward(ship_id):
    ship_type_key = to_space_object(ship_id).art_id
    ship_blob = to_blob(ship_id)
    if ship_blob is None:
        return None
    if ship_type_key == ship_type_pirate_strongbow_ef():
        return ship_blob.get(blob_key_beam_angle(), 2) < 90
    elif ship_type_key == ship_type_pirate_brigantine_ef():
        return ship_blob.get(blob_key_beam_angle(), 0) < 55
    else:
        return None

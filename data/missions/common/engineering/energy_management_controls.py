...
def auto_set_manual_mode(player_ship_id):
    auto_set_checkbox_state(player_ship_id, switch_identifier_auto_mode(), False)
    set_inventory_value(player_ship_id, inventory_key_is_switch_on(switch_identifier_auto_mode()), False)
...
def set_switch_state(player_ship_id, switch_identifier, is_new_state_on, is_auto_mode_command):
    if is_auto_mode_command:
        if not get_inventory_value(player_ship_id, inventory_key_is_switch_on(switch_identifier_auto_mode())):
            return False
        auto_set_checkbox_state(player_ship_id, switch_identifier, is_new_state_on)
    else:
...
def auto_set_checkbox_state(player_ship_id, checkbox_identifier, on):
    signal_emit(signal_energy_management_controls_non_manual_change_checkbox(player_ship_id), data={ "SWITCH_IDENTIFIER": checkbox_identifier, "ON": on })
...
def signal_energy_management_controls_non_manual_change_checkbox(player_ship_id):
    return f"emc_non_manual_change_checkbox_{player_ship_id}"
